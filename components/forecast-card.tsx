...
export function ForecastCard({ forecast }: { forecast: ForecastDTO }) {
  ...
  // Defensive: older forecast rows (or a stale JSON column read) can hand back
  // `series` as null/undefined/an object instead of an array. Recharts throws
  // a cryptic "displayedData.map is not a function" if it's not a real array.
  const chartSeries = Array.isArray(forecast.series) ? forecast.series : [];
        ...
        <div className="h-16">
          {chartSeries.length > 0 ? (
            ...
            <div className="flex h-full items-center justify-center text-[10px] text-muted-foreground">
              No daily trend data for this forecast
            </div>
          )}
        </div>
